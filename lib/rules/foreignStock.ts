...
export class ForeignStockSplitSellRule extends BaseRule {
  ...
  evaluate(profile: UserProfile): Candidate {
    ...
    return {
      ...
      tax_rate_percent: Math.round(FOREIGN_TAX_RATE * 1000) / 10,
...
export class FamilyGiftRule extends BaseRule {
  ...
  isApplicable(profile: UserProfile): boolean {
    if (!profile.invest_types.includes('foreign_stock')) return false;
    if (profile.foreign_stock_unrealized_profit <= GIFT_MIN_UNREALIZED)
      return false;
  ...
  evaluate(profile: UserProfile): Candidate {
    ...
    if (profile.has_children) {
      if (profile.has_minor_children)
        targets.push(`미성년자녀 ${GIFT_MINOR}만원`);
    ...
    return {
      ...
      reason: `미실현 수익 ${profile.foreign_stock_unrealized_profit}만원 보유 주식을 가족(${targets.join(', ')})에게 증여 후 매도 시 증여 시점 시가가 취득가액으로 재설정되어 양도세 절감 가능. 10년 합산 비과세 한도 내에서 가능.`,
