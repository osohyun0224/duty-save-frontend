...
export class PensionRule extends BaseRule {
  ...
  evaluate(profile: UserProfile): Candidate {
    ...
    return {
      ...
      reason: `연봉 ${profile.annual_salary}만원 기준 세액공제율 ${(taxRate * 100).toFixed(1)}% 적용. 현재 납입 ${profile.pension_contribution}만원, 한도 ${PENSION_LIMIT}만원까지 ${room}만원 여유. ${fill}만원 추가 납입 시 약 ${benefitManWon}만원 환급.`,
